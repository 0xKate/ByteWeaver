        ...
        __try {
            ...
            if (result == NO_ERROR) {
                ...
                if constexpr (ENABLE_DETOUR_LOGGING) {
                    if (this->Key) {
                        Debug("[Detour] (Apply) [Target: " ADDR_FMT " -> Detour: " ADDR_FMT " Size: %zu, Key: %s]",  TargetAddress, DetourFunction, Size, Key);
                    } else {
    ...
    bool Detour::Restore()
    {
        ...
        __try {
            ...
            if (result == NO_ERROR) {
                ...
                if constexpr (ENABLE_DETOUR_LOGGING) {
                    if (this->Key) {
                        Debug("[Detour] (Restore) [Target: " ADDR_FMT " -> Detour: " ADDR_FMT " Size: %zu, Key: %s]",  TargetAddress, TargetAddress, Size, Key);
                    } else {
